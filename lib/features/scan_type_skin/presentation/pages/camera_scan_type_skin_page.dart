...
class CameraScanTypeSkinPage extends StatefulWidget {
  const CameraScanTypeSkinPage({super.key});

  @override
  State<CameraScanTypeSkinPage> createState() => _CameraScanTypeSkinPageState();
}

class _CameraScanTypeSkinPageState extends State<CameraScanTypeSkinPage> {
  File? _capturedImage;
  File? selectedImage;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        body: BlocConsumer<ScanImageBloc, ScanImageState>(
      listener: (context, state) {
        if (state is ScanImageLoading) {
          showLoadingDialog(context);
        } else if (state is ScanImageFailed) {
        ...
        } else if (state is ScanImageLoaded) {
          Future.microtask(() {
            if (mounted) {
              Navigator.pushNamedAndRemoveUntil(
                context,
                AppRoute.resultScanImagePage,
                (route) => false,
                arguments: ScanImageResultArgument(
                    scanImage: state.data, file: _capturedImage),
              );
            }
          });
        }
              ...
              children: [
                ...
                FilledButtonItems(
                  ...
                  onPressed: () {
                    ...
                    context
                        .read<ScanImageBloc>()
                        .add(GetScanImageEvent(image: _capturedImage!));
                  },
                )
              ],
            )),
